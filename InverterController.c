...
//MRAS初始化
	initMRAS(&pInvCon->MRAS_theta);	
//转速pi初始化
	pInvCon->Pidn.Kp  = 0.5;
	pInvCon->Pidn.Ki  = 60;
	pInvCon->Pidn.Ts  = CON_TS;
	pInvCon->Pidn.LimitHigh = 100;
	pInvCon->Pidn.LimitLow  = -100;

	
//dq轴电流初始化初始化
	pInvCon->Pid_iq.Kp = 100;
	pInvCon->Pid_iq.Ki = 15000;
	pInvCon->Pid_iq.Ts = CON_TS;
	pInvCon->Pid_iq.LimitHigh = 350;
	pInvCon->Pid_iq.LimitLow = -350;
	
	pInvCon->Pid_id.Kp = 100;
	pInvCon->Pid_id.Ki = 20000;
...
//SMO
	...
	// if (i<500)
	// {
	// 	pInvCon->SMO_theta.theta = 0;
	// 	i++;
	// }

//MRAS
	pInvCon->MRAS_theta.id = pInvCon->CurPark.Ud;
	pInvCon->MRAS_theta.iq = pInvCon->CurPark.Uq;
	pInvCon->MRAS_theta.Ud = pInvCon->VolPark.Ud;
	pInvCon->MRAS_theta.Uq = pInvCon->VolPark.Uq;
	calcMRAS(&pInvCon->MRAS_theta);
	// if (i<500)
	// {
	// 	pInvCon->MRAS_theta.theta = 0;
	// 	i++;
	// }
...
//转速控制器
	...
	// pInvCon->Pidn.Ref = pInvCon->Targetn;
	pInvCon->Pidn.Fdb = pInvCon->MRAS_theta.omegae*rad2rpm/Pn;
...
//电压反变换
	pInvCon->VolIPark.Ud    =  pInvCon->Pid_id.PidOut ;
	pInvCon->VolIPark.Uq    =  pInvCon->Pid_iq.PidOut ;
	pInvCon->VolIPark.theta =  pInvCon->MRAS_theta.theta ;
...
void calcDspInterrupt(double *Uabc, double *Iabc, double *ref_id_n,double *theta_n ,double *PWMRef, double *Display_id, double *Display_n, double *Display_iq,double *Display)
{
	...
	Display[0]    = InverterControl.MRAS_theta.theta;
	Display[1]    = InverterControl.MRAS_theta.omegae*rad2rpm/Pn;
	Display[2]    = InverterControl.theta;
	Display[3]    = InverterControl.n;
	Display[4]    = InverterControl.MRAS_theta.Ud;
	Display[5]    = InverterControl.MRAS_theta.Uq;
	Display[6]    = InverterControl.MRAS_theta.id;
	Display[7]    = InverterControl.MRAS_theta.iq;
}