...
def get_axis(cNr):
    ...
    fig, axes = plt.subplots(ncols=cNr[0], nrows=cNr[1], sharex=True, figsize=(16*0.9, 9*0.9), dpi=80, facecolor='w', edgecolor='k');
...
def plot_key(ax, key, df):
    ax.plot(time, df[key].values, '-', lw=1,color='blue')
...
def plot_it(ax, ylabel, d, time=None):
    ...
    for k, v in d.items():
        if count == 0:
            ...
            ax.plot(time, v, '-', lw=1,color='blue')
        else:
            # ax.plot(time, v, '-', lw=2, label=k)
            ax.plot(time, v, '-', lw=1,color='blue')
